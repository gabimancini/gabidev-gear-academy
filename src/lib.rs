...
unsafe extern "C" fn handle() {
    ...
    if finished {
         debug!("The game is over and the winner is: {:?}", game_state.winner.unwrap());
...
unsafe fn exec_program_turn() -> u32 {
    ...
    debug!("Number of stones remaining: {}", game_state.pebbles_remaining);
    count
}
...
unsafe fn take_hard_action() -> u32 {
 ...
 unsafe fn remove_pebbles(count: u32) -> Option<()>{
    ...
    if count == 0 {
        debug!("The number of stones entered by the user cannot be 0, and the user needs to re-enter");
        return None; 
    }
